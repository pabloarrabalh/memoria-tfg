...
\subsubsection{7.1.1 Bloque de cliente: \textit{frontend}}
...
El \textit{frontend} es la capa de presentación encargada de renderizar la interfaz de
...
\subsubsection{7.1.2 Punto de entrada: \textit{Caddy}}
 
\textit{Caddy} se configura como el punto de entrada principal al servidor, actuando como intermediario
...
En este sentido, \textit{Cadyy} funciona como un proxy inverso, recibiendo las solicitudes y
...
\subsubsection{7.1.3 Núcleo del sistema: \textit{backend}}
El \textit{backend} constituye la capa de aplicación desarrollada en \textit{Django} y actúa como el núcleo que
...
y la comunicación con el motor de \textit{Machine Learning} para la obtención de predicciones y recomendaciones.
...
\subsubsection{7.1.4 Patrones empleados}
...
framework \textit{Django} bajo la variante Model-Template-View (MTV). Este patrón
...
\subsubsection{7.1.5 Patrón cliente-servidor}
...
\textit{frontend} (desarrollado en \textit{React}) y el \textit{backend} (\textit{Django}) operen como entidades
independientes.
...
\subsubsection{7.1.6 Arquitectura en capas}
...
\begin{itemize}
\item \textbf{Capa de presentación}: \textit{frontend-web} (\textit{React}).
\item \textbf{Capa de servicios/\textit{API}}: api/views (\textit{Django REST Framework}).
...
\subsubsection{7.1.7 Patrones de comportamiento}
...
\subsubsection{7.1.8 Observer}
 
El patrón \textit{Observer} define una relación uno-a-muchos donde múltiples objetos
(observadores) son notificados automáticamente ante cambios de estado en un
objeto sujeto.
 
\textbf{Aplicación en el sistema:}
 
Se implementa mediante el sistema de \textit{signals} de \textit{Django} (\texttt{signals.py}):
\begin{itemize}
\item Creación automática de UserProfile tras la creación de un User.
\item Indexación de entidades (Jugador, Equipo) en \textit{MeiliSearch}.
\item Emisión de notificaciones en tiempo real mediante \textit{WebSockets} (Django
Channels), utilizando \textit{Redis} como backend.
\end{itemize}
...
\subsubsection{7.1.9 Template method}
...
extienen dicha clase, sobrescribiendo etapas concretas del \textit{pipeline}.
\end{itemize}
...
\subsubsection{7.1.11 Patrones en el \textit{frontend}}
El diseño del \textit{frontend} no solo responde a criterios visuales, sino también a
...
\textit{React} que permiten construir interfaces modernas, escalables y mantenibles.
A continuación, se describen los principales mecanismos utilizados en el
\textit{frontend}, centrados en la gestión del estado global y la encapsulación de lógica
reutilizable.
...
\textit{frontend} sea reactivo y eficiente, alineándose con los estándares actuales de la industria y
garantizando una experiencia de usuario fluida.
 
\subsubsection{7.1.12 Context API}
 
\textit{Context API} es un sistema nativo de \textit{React} diseñado para compartir datos de forma global entre componentes,
...
mejorando la escalabilidad y mantenibilidad del \textit{frontend}.
...
\subsection{7.6 Diseño de la interfaz}
...
\subsubsection{7.6.1 Nivel de navegación}
...
\subsubsection{7.6.2 Estructura base y componentes persistentes}
...
\item \textbf{Sidebar (menú lateral)}: Funciona como el enrutador principal.
 Está permanentemente accesible y proporciona saltos directos a los
módulos core del sistema: Menú, Mi Plantilla, Liga, Amigos, Equipos y Estadísticas.
\end{itemize}
 
\subsubsection{7.6.3 Flujos de profundización}
...
\subsubsection{7.6.4 Diseño de la interfaz}
...
\subsection{7.7 Diseño de seguridad y autenticación}
...
\subsection{7.8 Diseño del despliegue}
...
\subsubsection{7.8.1 Ecosistema de contenedores y responsabilidades}
...
\subsubsection{7.8.2 Estrategia de construcción}
