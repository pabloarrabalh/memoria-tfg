...
PostgreSQL, búsqueda y recuperación documental en OpenSearch, y almacenamiento
...
\subsubsection*{Núcleo del sistema: backend (API)}
...
Learning.
\subsection*{7.1.2 Patrones empleados}
...
\subsection*{7.1.3 Patrones de comportamiento}
...
\item Indexación de entidades (Jugador, Equipo) en OpenSearch.
...
(indexación y notificaciones), mejorando la extensibilidad. La incorporación de
Redis en la capa de mensajería refuerza la escalabilidad horizontal, ya que los
eventos pueden propagarse de forma consistente entre procesos y nodos backend
...
\subsection*{7.1.4 Patrones en el frontend}
...
Los hooks permiten encapsular y reutilizar lógica de estado y ciclo de vida en
...
Favorecen la reutilización de lógica y una mayor claridad en los componentes,
alineándose con las buenas prácticas del desarrollo moderno en React.


\subsubsection*{Context API}

Mecanismo para gestionar estado global sin necesidad de prop drilling.
...
\subsection*{7.2 Pipeline de datos}
El núcleo técnico del sistema reside en la transformación de datos deportivos
brutos en conocimiento accionable. Este proceso se articula mediante un pipeline
de tipo ETL (Extract, Transform, Load), compuesto por diversas fases que abarcan
desde la ingesta de datos hasta su procesamiento y utilización en modelos
predictivos.
Cada una de estas etapas ha sido diseñada para garantizar la calidad,
consistencia y utilidad de los datos, permitiendo alimentar de forma fiable los
algoritmos de Machine Learning.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/etl.png}
\caption{Pipeline de ETL.}
\label{fig:etl}
\end{figure}


\subsection*{7.2.1 Ingesta}
Para la construcción del dataset, se han seleccionado dos fuentes de datos
heterogéneas que ofrecen visiones complementarias del dominio:

\begin{itemize}
\item \textbf{fbref.com}: Aporta métricas avanzadas de rendimiento (xG, PSxG,
conducciones progresivas, intercepciones, etc.).
\item \textbf{futbolfantasy.com}: Proporciona el contexto específico del juego
``Fantasy'', incluye los puntos obtenidos.
\end{itemize}


\subsection*{7.2.2 Conciliación de entidades}
Uno de los mayores retos técnicos es la desambiguación de nombres. Un mismo
jugador puede aparecer como "Lamine Yamal", "L. Yamal" o "Yamal" según la
fuente.
Para resolver esto, se implementó un sistema de Fuzzy Matching basado en la
distancia de Levenshtein. Este algoritmo calcula el número mínimo de ediciones
(inserciones, eliminaciones o sustituciones) necesarias para transformar una
cadena de texto en otra.
$$Distancia(s_1, s_2) = \min(\mathrm{cambios\ para\ } s_1 \rightarrow s_2)$$

Este proceso, junto con diccionarios para casos especiales de alias (ej: Pepelu
vs. José Luis García Vayá), permite asignar correctamente a todos los jugadores
sus puntos obtenidos en cada partido.

\subsection*{7.2.3 Limpieza y normalización de datos}
Una vez consolidado el histórico (2.5 temporadas), el script de procesamiento
realiza tareas críticas de saneamiento:

\begin{itemize}
\item \textbf{Tratamiento de outliers}: Se detectaron registros con puntuaciones
anómalas (p. ej., puntos > 30 en una jornada). Se decidió sustituir estos
valores por la mediana del conjunto de datos para evitar que el modelo de
Machine Learning aprenda de eventos extremadamente improbables que sesguen la
predicción.
\item \textbf{Filtrado de actividad}: Para asegurar la calidad de la señal, se
eliminan registros de jugadores con menos de 10 minutos disputados en un
partido, ya que su aporte estadístico no es representativo.
\end{itemize}

\subsection*{7.2.4 Ingeniería de características}
Esta fase es la más determinante para la precisión del sistema. No se entrena el
modelo con datos brutos, sino con variables elaboradas que capturan tendencias.

\subsubsection*{7.2.4.1. Enriquecimiento de roles (FBRef)}
Se procesan los JSON de roles de FBRef para calcular un score de importancia por
posición. La decisión de diseño clave aquí es la aplicación de un factor de
posición exponencial. Si un jugador está en el top 1 de una métrica, su peso es
mucho mayor que si está en el top 10. La fórmula aplicada es:
$$Score = \sum (Valor \times Peso \times \frac{1}{\sqrt{Posicion}})$$




\subsubsection*{7.2.4.2 Variables temporales (rolling y EWMA)}
Para capturar el estado de forma, se generan dos tipos de métricas sobre las
estadísticas base (puntos, paradas, entradas, etc.):

\begin{itemize}
\item \textbf{Rolling Mean (media móvil)}: Promedio simple de las últimas $N$
jornadas (ventanas de 3 y 5).
\item \textbf{EWMA (media móvil ponderada exponencialmente)}: Da más importancia
a los partidos más recientes, permitiendo al modelo reaccionar rápido a rachas
de buen juego.
\end{itemize}
$$EWMA_t = \alpha \cdot X_t + (1-\alpha) \cdot EWMA_{t-1}$$

\subsubsection*{7.2.4.3 Integración de clasificación y cuotas}
Para realizar la predicción de la jornada N
N, el sistema utiliza información de la jornada anterior (N-1), incorporando tanto la 
clasificación del equipo como la de su rival. De este modo, se introduce un contexto 
temporal que permite reflejar mejor el estado reciente de ambos conjuntos.

Además, se integran las cuotas de mercado, transformadas en probabilidades implícitas 
de victoria y de goles en contra. Esto aporta una estimación del “sentimiento del mercado” 
previa al partido, enriqueciendo el modelo con información externa relevante. Estos datos se
descargan desde: \href{https://www.football-data.co.uk/spainm.php}{https://\linebreak www.football-data.co.uk/spainm.php}

\subsection*{7.2.5 Prevención del data leakage}
En predicción deportiva, es común cometer el error de usar datos del ``futuro''
para predecir el ``pasado''. Para evitarlo, el sistema implementa tres
salvaguardas:

\begin{itemize}
\item \textbf{Desplazamiento de variables}: Todas las estadísticas de rendimiento se
desplazan una jornada hacia atrás (\texttt{shift(-1)}). Esto garantiza que para predecir los
puntos de la Jornada 20, el modelo solo vea estadísticas generadas hasta la
Jornada 19.
\item \textbf{Validación temporal}: En lugar de una validación aleatoria
(K-Fold), se utiliza un corte temporal: se entrena con el pasado y se testea
con el futuro más reciente.
\item \textbf{Posiciones desplazadas}: La posición en la tabla que ve el modelo es la
que el equipo tenía antes de empezar el partido, no la resultante tras jugar.
\end{itemize}



\subsection*{7.3 Modelado predictivo}
...
\subsubsection*{7.3.1. Algoritmos y optimización}
...
\subsection*{7.3.2 Métricas de evaluación del modelo}
...
\subsubsection*{7.3.2.1. Justificación técnica del coeficiente de Spearman}
...
\subsubsection*{7.3.3. Criterios de validez y benchmarking de modelos}
...
entrenamiento, se ha realizado un estudio de referencia (benchmarking) basado en
...
extremas de 15 o 20 puntos), se ha descartado el uso de la desviación estándar
...
\subsection*{7.4 Decisiones de diseño estratégicas}
...
\subsubsection*{7.4.1 Estrategia de extracción y congelación de datos}

Se ha optado por congelar la extracción de datos en la jornada 17 de la
temporada 2026/27. Esta decisión se fundamenta en los siguientes pilares:
...
\subsubsection*{7.4.2 Especialización de modelos por posición}
...
posición, por lo que se considera esta opción lo mejor para aportar un valor y
abordar el problema de la forma más realista posible.
...
\subsection*{7.5 Modelado de datos}
...
\subsection*{7.5.1 Modelado conceptual}
...
\subsubsection*{7.5.2 Diagrama entidad-relación}
...
entidades interactúan entre sí, se observan claramente las cardinalidades.
...
\subsection*{7.5.3 Modelo lógico}
...
\subsection*{7.6 Infraestructura de datos y búsqueda}
...
especializadas. Cada una de ellas desempeña un papel fundamental en la gestión,
...
\item \textbf{OpenSearch}: Se utiliza como motor de indexación y búsqueda
...
Dentro del diseño del sistema se ha incorporado un motor de búsqueda
especializado con el objetivo de complementar la base de datos relacional y
mejorar la eficiencia en la explotación de la información. Para ello, se ha
optado por el uso de OpenSearch como solución de indexación y consulta.
...
dependencia de licencias propietarias.

Desde el punto de vista del diseño, OpenSearch no reemplaza la base de datos
relacional, sino que actúa como un sistema complementario con un propósito
claramente diferenciado:

La base de datos relacional se encarga del
almacenamiento estructurado y la integridad de los datos.OpenSearch se utiliza 
para la indexación y consulta eficiente de
información, especialmente en escenarios de búsqueda avanzada, autocompletado y
análisis.

La decisión de incorporar OpenSearch responde a varias necesidades del sistema:
\begin{itemize}
\item \textbf{Optimización de consultas complejas}: determinadas operaciones, como
búsquedas por texto o filtrados múltiples, resultan costosas en bases de datos
relacionales.
\item \textbf{Escalabilidad en lectura}: el sistema está orientado a la consulta
intensiva de datos (estadísticas, rankings, comparativas), lo que hace
recomendable el uso de un motor especializado.
\item \textbf{Flexibilidad en la indexación}: permite almacenar documentos con
estructuras más flexibles, adaptadas a las necesidades de visualización y
análisis.
\item \textbf{Independencia tecnológica}: al tratarse de una solución open source,
 se evita la dependencia de licencias de pago.
\end{itemize}



\subsection*{7.8 Diseño de la interfaz}
...
\subsubsection*{7.8.1 Nivel de navegación}
...
\subsubsection*{7.8.2 Estructura base y componentes persistentes}
...
la gestión de la cuenta (Perfil), dependiendo de sesión activa
del usuario.
...
\subsection*{7.8.3 Flujos de profundización}
...
hacia el Detalle de Amigo. 

Por otro lado, la vista de Mi Plantilla no contiene
...
\subsubsection*{7.8.4 Diseño de la interfaz}
El diseño de la paleta de colores responde a criterios de simplicidad, contraste y coherencia visual.
 ...
 de tecnología, modernidad e innovación, valores que alinean 
...
\hline
\textbf{Elemento} & \textbf{Tamaño} & \textbf{Peso} & \textbf{Uso asignado} \\
\hline
h1 & 32px & Bold & Título de página principal \\
\hline
h2 & 24px & SemiBold & Título de sección \\
\hline
h3 & 20px & Medium & Subtítulo, encabezados secundarios \\
\hline
body & 16px & Regular & Texto principal, párrafos \\
\hline
body small & 14px & Regular & Ayudas, notas, etiquetas \\
\hline
caption/meta & 12px & Regular & Fechas, detalles secundarios \\
\hline
\end{tabular}
\caption{Jerarquía visual de tipografía.}
...
\subsection*{7.9 Diseño de seguridad y autenticación}
...
\subsection*{7.10 Diseño del despliegue}
...
\subsubsection*{7.10.1 Ecosistema de Contenedores y Responsabilidades}

El sistema se descompone en cinco servicios principales orquestados mediante una
red virtual privada:
...
\item \textbf{Motor de búsqueda (opensearch)}: nodo de OpenSearch 2.11
encargado de la indexación y consultas de alto rendimiento. Incluye
healthchecks mediante curl para asegurar la disponibilidad del clúster antes de
iniciar el backend.
...
\end{figure}
\subsubsection*{7.10.2 Estrategia de Construcción}
...
Se han definido tres entornos diferenciados para cubrir todas las fases del
desarrollo:
...
\item \textbf{Entorno de Preproducción (Render)}: se utiliza la plataforma
Render para despliegues continuos de prueba. Este entorno sirve para validar la
integración de los modelos de Machine Learning en un contexto de red real y
realizar pruebas de QA.
...
optimización crítica de recursos sin sacrificio de funcionalidad. La selección
 sistemática de versiones slim y alpine para las imágenes base, junto con una
