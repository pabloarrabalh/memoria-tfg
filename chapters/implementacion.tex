...
integridad (por ejemplo, evitar duplicar estadísticas de un jugador en un mismo partido), 
así como validadores en múltiples campos. Además, se utilizaron campos JSON en varios modelos
 (como percentiles o datos de notificaciones) para permitir mayor flexibilidad sin necesidad de 
...
\begin{lstlisting}[language=Python,caption={Calcular posición más frecuente de un jugador}]
...
\begin{description}
\item[Serialización:] Convertir objetos Python/ORM a JSON y validar datos entrantes.
\item[Autenticación y permisos:] Verificar quién es el usuario y qué puede hacer.
\item[Negociación de contenido:] Servir JSON, XML, etc. según lo que pida el cliente.
\item[Paginación:] Fragmentar resultados de 10.000 registros en bloques.
\item[Filtrado y búsqueda:] Aplicar criterios a consultas sin duplicar SQL.
\item[Estándares HTTP:] Retornar códigos de estado correctos (200, 400, 404, 500, etc).
\end{description}
...
\begin{description}
\item[API v1:] Endpoints desarrollados incrementalmente durante las primeras fases del
 proyecto. Conviven endpoints dispares como \texttt{/api/predecir-portero/}, 
 \texttt{/api/cambiar-jornada/}, \texttt{/api/plantilla/}, cada uno con su propia
  lógica de validación y respuesta. Aunque heterogénea, esta versión sigue activa 
  porque ciertas funcionalidades depende de ella.

\item[API v2 (RESTful):] Una arquitectura más sistemática usando ViewSets de DRF,
 con endpoints jerárquicos como \texttt{/api/v2/jugadores/}, \texttt{/api/v2/equipos/},
  \texttt{/api/v2/clasificacion/}. Sigue principios REST: operaciones CRUD sobre recursos
   claramente definidos, códigos HTTP semánticos, y paginación estándar.
\end{description}
...
Un serializer en DRF es una clase que define cómo se transforma un objeto de Python
 ...
 viceversa. Es el punto donde se establece el ``contrato'' entre el servidor y el cliente:
  ``los datos que espero recibir/enviar tienen esta estructura exacta, con estos tipos,
   y cumplen estas validaciones''.
...
La gestión de amistades requiere una lógica que identifique correctamente a los participantes de la relación
 ...
 petición (request) para determinar cuál de los dos usuarios en el modelo Amistad es el ``amigo'' del usuario
...
\begin{figure}[h]
\centering
\includegraphics[width=0.1\textwidth]{./images/api.png}
...
Este bloque constituye el núcleo analítico del sistema, encargándose de transformar volúmenes masivos de métricas en bruto en información estructurada,
 ránkings comparativos y narrativas de partido. El módulo \texttt{buscar.py} actúa como la interfaz de abstracción sobre el motor OpenSearch, implementando
...
sanitiza proactivamente la salida para evitar errores de parsing en el cliente.
...
Además, el módulo \texttt{estadisticas.py} implementa un motor de filtrado de outliers estadísticos, sustituyendo puntos anómalos (>40) por la moda
 del conjunto y calculando percentiles relativos por posición para asegurar comparativas justas.
...
validando formaciones tácticas (ej. 4-3-3) y calculando puntos en tiempo real. Se apoya en \texttt{plantilla\_notificaciones.py}, un monitor reactivo 
que detecta eventos en vivo (goles, tarjetas o asistencias) de los jugadores alineados por el usuario mediante un sistema de claves únicas 
...
Finalmente, como medida de seguridad deportiva, la API rastrea la participación 
...
El archivo \texttt{menu.py} actúa como el orquestador principal de la vista de inicio. Aunque se implementó una
 función \texttt{\_bg\_compute\_predictions} para procesamiento paralelo, el flujo actual del \texttt{MenuView} 
 devuelve las predicciones síncronamente desde el caché a través del decorador \texttt{@cache\_api\_response}, que almacena 
 resultados durante 120 segundos. Las predicciones se pre-generan al arranque del sistema mediante el pipeline de \texttt{apps.py} (paso 5), 
 garantizando disponibilidad inmediata desde el primer minuto de operación. Para solicitudes posteriores dentro de la ventana de caché, 
 se sirven datos almacenados sin cálculos adicionales.

Aquellas vistas con una carga algorítmica especialmente relevante serán analizadas con mayor profundidad en los capítulos correspondientes de esta memoria.

\subsection*{8.1.3 Arquitectura reactiva y optimización de rendimiento}
...
\begin{itemize}
\item Indexación en OpenSearch: Cada vez que un Jugador o Equipo es creado o actualizado, una señal \texttt{post\_save} actualiza automáticamente el índice en el motor de búsqueda, garantizando que los resultados de búsqueda estén siempre sincronizados.
...
\subsection*{8.1.4 Arranque y ciclo de vida}
...
asegurando que todos los servicios dependientes (Base de Datos, OpenSearch y Modelos de ML) estén sincronizados y listos para operar.
...
dispara un hilo demonio que ejecuta una tubería (pipeline) de 5 pasos críticos 
sin impedir que el servidor comience a escuchar peticiones.

\begin{lstlisting}[language=Python,caption={Inicialización 
asíncrona en hilo demonio durante startup de Django}]
def ready(self):
...
\item Sincronización con el motor de búsqueda: verifica la disponibilidad del cliente OpenSearch y fuerza una reindexación completa de jugadores y equipos.
...
\begin{description}
\item[FBRef:] Fuente principal de métricas avanzadas (xG, PSxG, progresiones, estadísticas defensivas y de portería).
\item[FutbolFantasy:] Utilizada exclusivamente para la obtención de la ``verdad'' del juego: los puntos obtenidos por cada jugador en la plataforma oficial.
\item[Transfermarkt:] Fuente de contexto competitivo, empleada para extraer las rachas de los equipos, datos de estadios y nacionalidades.
\end{description}
...
\item Bypass de Cloudflare en FutbolFantasy: el sitio bloquea agentes no humanos. Se implementó el módulo \texttt{descargar\_puntos.py} utilizando la librería \texttt{cloudscraper}, que permite simular una huella de navegador real, gestionando automáticamente los desafíos de JavaScript y cabeceras User-Agent.
\end{itemize}
...
reintentos con backoff exponencial para descargas robustas}]
...
\item Idempotencia: el sistema verifica la integridad y existencia de archivos locales antes de iniciar la descarga, permitiendo reanudar procesos interrumpidos sin redundancia de red.
\end{itemize}

\begin{lstlisting}[language=Python,caption={Backoff exponencial con varianza aleatoria para mitigación de detección de bots}]
...
Mientras FBRef aporta datos intrínsecos del jugador, este módulo recupera variables extrínsecas del equipo. Implementa la función \texttt{parse\_tabla\_jornada\_transfermarkt()} que
 extrae de forma confiable:

\begin{itemize}
\item Posición en clasificación (PJ, PG, PE, PP, GF-GC, puntos)
\item Diferencia de goles (DG) y ratio de eficiencia
\end{itemize}

\paragraph*{E. roles.py}

Módulo avanzado diseñado para detectar ``jugadores élite'' en categorías estadísticas específicas.

\begin{itemize}
\item Parseo de elementos Ocultos: muchos datos de liderazgo en FBRef se cargan de forma asíncrona o residen dentro de nodos de comentario HTML para optimizar el renderizado del navegador. \texttt{roles.py} implementa una técnica de extracción de comentarios mediante BeautifulSoup para acceder a esta información.
...
Los roles se almacenan en un objeto JSON donde la clave principal es el nombre normalizado del jugador, facilitando un acceso rápido durante el entrenamiento.
 Cada registro contiene una lista de los ránkings donde el jugador es protagonista, almacenando la posición (rank) y el valor absoluto de la métrica.
...
\subsubsection*{8.2.6 Orquestación de la canalización de scraping}
...
\item Limpieza de minutos: FBRef a veces incluye el minuto del evento en el 
nombre del jugador en el HTML; el sistema lo detecta y limpia mediante Regex.
\end{itemize}

\begin{lstlisting}[language=Python,caption={Normalización de 
texto y limpieza de minutos para entity matching}]
...
\subsubsection*{8.3.2 Alias por temporada y casos especiales}

Existen casos donde la normalización no es suficiente (ej. ``Pepelu'' no se parece a ``José Luis García Vaya''). El sistema utiliza un diccionario de
 alias que mapea nombres conocidos según la temporada y el equipo. Esto permite interceptar estos casos antes de que lleguen a la lógica probabilística.

Ejemplos de flujo:

\begin{itemize}
\item Isaac Palazón Camacho $\rightarrow$ Isi (Rayo Vallecano)
\item Alexander Sørloth $\rightarrow$ Sorloth (Villarreal/Atlético)
\end{itemize}

\subsubsection*{8.3.3 Generación de propuestas de matching}
...
\subsubsection*{8.3.4 Lógica principal y Fuzzy Matching}

La función \texttt{obten\_coincidencia\_nombre} es el corazón del módulo. Utiliza un enfoque híbrido:

\begin{itemize}
\item Regla de Apellidos Críticos: Si el apellido es muy común (ej. ``García'', ``González''), el sistema bloquea los matches parciales para evitar confusiones entre compañeros de equipo (como los hermanos Williams en el Athletic).

\item Match por Prefijo/Sufijo: Si el nombre es un solo token y no es crítico, se busca si coincide con el inicio o fin de un nombre en la base de datos.

\item Algoritmo WRatio (RapidFuzz): Si las reglas anteriores fallan, se recurre a \texttt{fuzz.WRatio}.
\end{itemize}

\subsubsection*{8.3.4.1 Justificación técnica: ¿Por qué WRatio frente al método tradicional?}

En la comparación de cadenas de texto, el método más común es la Distancia de Levenshtein, que mide cuántos cambios letra a letra hay que hacer 
para que una palabra sea igual a otra. Sin embargo, para un sistema de datos deportivos, este método es demasiado rígido: si un nombre está desordenado
 o le falta un apellido, Levenshtein daría una puntuación de coincidencia muy baja.

Para solucionar esto, el sistema utiliza el algoritmo WRatio (Weighted Ratio) de la librería RapidFuzz. Este no es un único cálculo, sino una combinación inteligente de varios ``sub-algoritmos'' que permiten al sistema comportarse de forma más humana:

\begin{itemize}
\item Partial Ratio (Efecto ``Subcadena''): ideal para cuando una fuente usa el nombre completo y otra uno corto. Si comparamos ``Isaac Palazón'' con ``Isaac Palazón Camacho'', el algoritmo detecta que la primera cadena está totalmente contenida en la segunda. Mientras que un método tradicional restaría muchos puntos por los caracteres que faltan, el Partial Ratio identifica que es un match casi seguro.

\item Token Sort Ratio: este es el paso clave para los nombres desordenados. El algoritmo descompone el nombre en palabras (tokens), las ordena alfabéticamente y luego las compara. Ejemplo: ``García Joan'' y ``Joan García'' se convierten internamente en la misma secuencia (``García'', ``Joan''), logrando una coincidencia del 100\%.

\item Escalado de pesos: el prefijo ``W'' indica que el algoritmo es capaz de ponderar qué método es más fiable según la longitud de los nombres y el tipo de diferencias encontradas, devolviendo una puntuación final de 0 a 100 mucho más realista.
\end{itemize}

Implementación del Fallback:

\begin{lstlisting}[language=Python,caption={Fuzzy matching con 
algoritmo WRatio para coincidencia de nombres robusta}]
...
Gracias a este enfoque, el sistema reduce drásticamente los ``falsos negativos'' (jugadores que existen en ambas bases pero que el ordenador no 
reconoce por una tilde o un apellido de más), garantizando un dataset final mucho más robusto para el entrenamiento del modelo.

\subsubsection*{8.3.5 Resolución de conflictos y desempates}

A veces, la lógica difusa puede proponer un mismo jugador de Fantasy para dos registros distintos de FBRef. El sistema resuelve estas 
colisiones en \texttt{resolver\_matching} usando criterios de negocio:

\begin{itemize}
\item Prioridad por Minutos: si un jugador de Fantasy tiene varias propuestas, se le asigna la que tiene más minutos jugados en FBRef (evitando registros residuales o errores de rotulación).

\item Prioridad por Puntos: si hay varios candidatos Fantasy similares, se ordenan por sus puntos en la jornada para emparejar primero a los que tuvieron participación real.

\item Consistencia de Apellido: en casos de conflicto, se valida que el apellido de la propuesta coincida con el del candidato antes de validar el score.
\end{itemize}

\begin{lstlisting}[language=Python,caption={Resolución de conflictos de matching por criterios de minutos y puntuación}]
if len(candidatos_ff_ordenados) == 1 and len(lista_props_ordenada) > 1:
    ...
    asignacion_fbref_a_fantasy[mejor_prop["clave_fbref"]] = candidato["clave_ff"]
\end{lstlisting}

\subsubsection*{8.3.6 Integración con el flujo de Scraping}

El matching no es un proceso aislado, sino que está integrado en el pipeline de FBRef:

\begin{itemize}
\item Extracción: se parsean las tablas de summary, passing, defense, etc.
\item Propuestas: se ejecuta \texttt{generar\_propuestas} para cada equipo.
\item Resolución: se consolidan las asignaciones y se genera el DataFrame final.
\item Casos Especiales (Banquillo): mediante \texttt{completar\_fantasy\_sin\_match}, se añaden aquellos jugadores que no aparecen en las estadísticas de juego de FBRef pero sí en Fantasy por haber recibido una tarjeta en el banquillo.
\end{itemize}
...
La persistencia de los artefactos se resolvió mediante la serialización en archivos .pkl (pickle). Se tomó la decisión estratégica de conservar
 siempre una versión de Random Forest como arquitectura de respaldo (fallback), incluso en escenarios donde modelos lineales como Ridge presentaran
  un MAE ligeramente inferior. Esta elección responde a la capacidad intrínseca de los modelos basados en árboles para manejar mejor la varianza y
   los datos ruidosos en entornos de producción en tiempo real.
...
Se seleccionaron el Error Absoluto Medio (MAE) y la Raíz del Error Cuadrático Medio (RMSE) para auditar la precisión de las predicciones en la unidad de medida del problema: los puntos Fantasy.
...
\begin{itemize}
\item Funcionamiento: a diferencia de Pearson, que evalúa relaciones lineales, Spearman realiza un mapeo de los valores brutos a sus correspondientes rangos o posiciones. El coeficiente cuantifica la disparidad entre el ranking predicho y el ranking real; una coincidencia exacta en la jerarquía de los jugadores resulta en una correlación de $+1$, validando la naturaleza monótona de la predicción.
...
Durante la fase de validación, se descartó el uso del coeficiente $R^2$ como criterio de selección. El motivo principal es la alta estocasticidad (azar) intrínseca del fútbol.
...
señales informativas de alto valor. El feature engineering se diseñó como una capa de transformación intermedia que combina la ingeniería de series temporales
 con la creación de variables sintéticas de dominio específico. Mediante la síntesis de métricas base, se han generado indicadores complejos de eficiencia y 
...
pesa mucho más que un décimo o un vigésimo. La segunda procede del contexto de partido, con variables derivadas de cuotas y dificultad de fixture, que ayudan 
a estimar el escenario competitivo antes del encuentro.

Este bloque fue especialmente útil para introducir señales de elite y de contexto sin depender solo de estadísticas acumuladas. Por ejemplo, 
...
en la Correlación de Spearman. A diferencia de Pearson, Spearman detecta relaciones no lineales (monótonas), lo cual es crucial en el fútbol donde el
 impacto de una estadística (ej. despejes) en los puntos suele saturarse a partir de cierto umbral. Se descartaron sistemáticamente todas las variables
...
Uno de los mayores retos al trabajar con modelos de Machine Learning complejos (como Ensembles o modelos con alta dimensionalidad) es su opacidad, 
conocida comúnmente como el problema de la ``caja negra''. Para un usuario de la plataforma, recibir una predicción de ``8.5 puntos'' sin contexto
...
El sistema utiliza la librería SHAP, fundamentada técnicamente en la Teoría de Juegos Cooperativos. Bajo este enfoque, la predicción final del modelo se
...
\begin{itemize}
\item Polimorfismo de Modelos: el sistema detecta automáticamente la arquitectura del modelo cargado. Si es un bosque de decisión (Random Forest o XGBoost), 
...
Mecanismo de Robustez (Fallback): en escenarios donde SHAP no puede inicializarse (por ejemplo, por falta de datos de fondo suficientes o incompatibilidad 
de versiones en los artefactos serializados), el sistema activa un fallback basado en la importancia intrínseca del modelo (\texttt{feature\_importances\_}) 
o en los coeficientes de regresión, asegurando que la API nunca devuelva una predicción sin su correspondiente justificación.
...
jornada 6, el orquestador conmuta dinámicamente al motor de Machine Learning y activa las explicaciones XAI. Además, el sistema inyecta alertas 
...
\item ClasificacionPage: muestra la tabla clasificatoria y permite consultar el estado de la liga por jornadas.
...
\item useJornada: hook que permite a cualquier componente suscribirse a los cambios de la jornada activa sin necesidad de pasar props a través de múltiples niveles de la jerarquía (Prop Drilling).
...
Casos como los de los hermanos Williams (Iñaki y Nico, I. Williams y N. Williams) en el Athletic Club, o porteros con nombres extremadamente comunes como Eric García y Joan García, generaban colisiones en los algoritmos de búsqueda. 
