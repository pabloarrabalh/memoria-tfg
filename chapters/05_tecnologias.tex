...
híbrido: construir una aplicación web completa e
integrar un motor de análisis de datos y \textit{Machine Learning} con capacidad de 
...
vida del dato y su exposición a través de 
una API web.
...
una separación estricta entre \textit{frontend}, \textit{backend}/\textit{API} y motor de procesamiento de 
...
\begin{itemize}
\item \textbf{Capa de presentación (\textit{Frontend}):} desarrollada con \textit{React} y \textit{Vite}. Se 
...
\item \textbf{Capa de aplicación / \textit{API} (\textit{Backend}):} implementada con \textit{Django} 5.1.4 + 
\textit{Django REST Framework} (DRF). Esta capa encapsula la lógica de negocio, la 
...
construir APIs, reduciendo boilerplate y estandarizando el diseño de endpoints.

\item \textbf{Capa de procesamiento:} motor especializado en \textit{Python} que integra 
\textit{Pandas}/\textit{NumPy}, librerías de ML (\textit{scikit-learn}, \textit{XGBoost}, \textit{LightGBM}) y de explicabilidad 
(\textit{SHAP}). Esta capa puede ejecutarse como módulo interno del backend o como 
...
\textit{PostgreSQL} (entorno de producción) y \textit{SQLite} (entorno de desarrollo). Para 
producción se prioriza \textit{PostgreSQL} por su robustez y su soporte de transacciones 
(con propiedades ACID), importante para consistencia bajo concurrencia y operaciones 
críticas. Además, se integra Redis como soporte de baja latencia y capa de caché.
\end{itemize}

\textbf{Comunicación entre capas}

\begin{itemize}
\item \textit{Frontend} $\leftrightarrow$ \textit{Backend}/\textit{API}: comunicación \textit{HTTP} usando \textit{JSON} como 
formato principal.

\item \textit{Backend} $\leftrightarrow$ Persistencia: acceso mediante el ORM de \textit{Django}, que 
traduce modelos del dominio a tablas relacionales, y permite migraciones y control 
de integridad de forma ordenada.

\item \textit{Backend}/\textit{API} $\leftrightarrow$ Motor ML: integración directa en \textit{Python} para 
...
La decisión de emplear \textit{Python} como lenguaje principal se fundamenta en razones técnicas y de 
eficiencia en el desarrollo: el proyecto combina desarrollo web y un componente significativo de 
análisis de datos y \textit{Machine Learning}. En este contexto, \textit{Python} destaca por su madurez en el 
ecosistema de ciencia de datos y por permitir construir 
...
Node.js habría permitido unificar el lenguaje en \textit{frontend} y \textit{backend}, pero carece de un ecosistema 
...
El \textit{backend} se implementa con \textit{Django} 5.1.4 por una combinación de productividad, estructura y 
seguridad. \textit{Django} proporciona una base sólida para organizar el proyecto (apps, modelos, vistas, 
etc.), y DRF (\textit{Django REST Framework}) se incorpora para exponer la funcionalidad mediante una \textit{API} 
\textit{REST}. Esto encaja con un \textit{frontend} desacoplado que consume endpoints, evitando dependencia 
...
En cuanto a seguridad, \textit{Django} incorpora protecciones y buenas prácticas documentadas para mitigar riesgos 
...
La elección de \textit{Django} 5.1.4 junto a \textit{Django REST Framework} responde a la necesidad de construir un 
\textit{backend} robusto, seguro y altamente modular. Mientras \textit{Django} proporciona la base estructural (gestión 
de base de datos, seguridad y administración), DRF se incorpora para transformar estas capacidades 
en una \textit{API RESTful} que sirve como el núcleo del sistema, promoviendo las buenas prácticas en el 
desarrollo software.
...
FastAPI: destaca por rendimiento, tipado y documentación automática; aun así, \textit{Django} aporta una 
...
\textit{PostgreSQL} es un sistema gestor de bases de datos relacional ampliamente adoptado en entornos 
...
Como complemento indispensable, se incorpora \textbf{Redis} como almacén de datos clave-valor en memoria. Su elección se justifica por dos necesidades estructurales:
\begin{itemize}
    \item \textbf{Soporte para Django Channels:} Redis funciona como la \textit{channel layer} necesaria para gestionar la comunicación asíncrona de los \textit{WebSockets}, permitiendo el envío de notificaciones en tiempo real (eventos de partidos o interacciones sociales).
    \item \textbf{Capa de caché:} se emplea para almacenar temporalmente los resultados de cálculos estadísticos complejos (percentiles y ránkings). Al servir estos datos desde la memoria RAM, se evita la recomputación constante y se optimiza el rendimiento de la API bajo concurrencia.
\end{itemize}
...
El proyecto incorpora un \textit{pipeline} de datos completo que no se limita a entrenar un modelo: incluye 
...
\item Generación de \textit{features} a partir de histórico (medias móviles, acumulados, forma reciente, 
local/visitante, etc.), cuidando que las \textit{features} respeten la causalidad temporal (evitar 
\textit{data leakage}).
\end{itemize}

El resultado del ETL es un dataset donde filas representan un hecho (p. ej., equipo en jornada, 
jugador en partido) y columnas representan variables. Esto se conoce como enfoque tabular y es coherente con la 
...
Se utiliza \textit{scikit‑learn} para evaluación y validación (splits, métricas, pipelines y comparación de 
...
\subsubsection{5.5.3 Interpretabilidad (XAI con \textit{SHAP})}

La interpretabilidad se aborda con \textit{SHAP} para generar explicaciones:
...
Se evalúa \textit{deep learning} (TensorFlow/PyTorch), pero para datos tabulares suele 
...
\subsubsection{5.5.4 Serialización e integración del modelo predictivo}

Para materializar la conexión entre la fase de entrenamiento y el entorno de 
operación, se hace uso del módulo pickle de Python. Una vez que el \textit{pipeline} de 
preprocesamiento y los modelos predictivos han sido entrenados y validados, el 
objeto resultante se serializa y se exporta como un archivo binario (.pkl), con la tecnología Pickle.
...
\subsubsection{5.6.1 Descarga de CSVs con cuotas}

Una fuente clave de cuotas de apuestas son los ficheros CSV públicos de \textit{Football‑Data.co.uk} 
...
La inclusión de datos externos de contexto se consideraba algo fundamental en el proyecto y su obtención
en fases iniciales fue escasa. En esa etapa se observó una diferencia aproximadada de 0.5 en el MAE, en función de si 
se tenían o no datos relacionados con las apuestas.
...
\textit{statsbomb}, \textit{soccerdata}), finalmente descartadas por limitaciones prácticas (cobertura, 
...
Dadas las limitaciones con las APIs, en la mayoría de casos se recurre a \textit{scraping} con requests y 
...
\subsection{5.7 \textit{Frontend} con \textit{React} y \textit{Vite}}

La capa de presentación se desarrolla con \textit{React} por su enfoque basado en componentes, idóneo para 
dashboards y vistas con alta reutilización (tarjetas de jugadores, tablas, filtros, gráficos, 
paneles de explicación). Se utiliza \textit{Vite} como herramienta de build y servidor de desarrollo, 
...
Inicialmente se valora usar templates del lado servidor (\textit{Django templates}), lo cual habría 
simplificado la arquitectura a corto plazo. Sin embargo, se opta por un \textit{frontend} desacoplado para 
...
\subsubsection{5.7.2 Base estructural: adaptación de \textit{React Petclinic}}

El proyecto toma como base estructural \textit{React Petclinic}, del cual se elimina toda la interfaz y 
...
Vue: productivo y simple, pero \textit{React} ofrece adopción industrial muy extendida y un ecosistema enorme 
para visualización y gestión de estado.

Templates del servidor: más sencillo al principio, pero limita escalabilidad de \textit{UI} y mezcla 
responsabilidades, complicando la evolución hacia un cliente rico.

\subsection{5.8 \textit{APIs} y otras herramientas de soporte}

Además de la \textit{API} principal (DRF), se incorporan herramientas complementarias para resolver 
...
combinan datasets externos y \textit{scraping}.

\textbf{Meilisearch:} se emplea como motor de búsqueda para habilitar consultas rápidas sobre campos 
de texto (nombres de jugadores/equipos). Se valoró también el uso de ElasticSearch y OpenSearch, pero fueron descartados por motivos técnicos.
...
en el stack de \textit{Machine Learning}), se ha optado por el uso de entornos virtuales mediante venv 
(Python Virtual Environments). Lo que resulta fundamental en varios aspectos:
...
el motor de ML y el \textit{backend} de \textit{Django} funcionen correctamente en cualquier máquina, se mantiene un 
...
\subsubsection{5.9.2 Contenedores}

Se ha implementado una arquitectura basada en contenedores utilizando \textit{Docker}, lo que permite aislar 
cada componente del sistema en un entorno controlado. Para la gestión de estos servicios, se emplea 
\textit{Docker Compose}, actuando como el orquestador que define y arranca los cuatro nodos principales que 
conforman la aplicación:

\begin{itemize}
\item \textbf{Backend (\textit{API}):} Contenedor basado en \textit{Python} 3.12 que aloja la lógica de \textit{Django} y el 
motor de \textit{Machine Learning}.

\item \textbf{Frontend (\textit{UI}):} Contenedor que sirve la aplicación \textit{React}, optimizada tras el proceso 
de \textit{build} de \textit{Vite}.

\item \textbf{Capa de mensajería y caché (\textit{Redis})}: Nodo independiente basado en \texttt{redis:7-alpine} para el soporte de tiempo real y optimización de lecturas de la \textit{API}.
\item \textbf{Persistencia (\textit{PostgreSQL})}: Servicio dedicado para la base de datos relacional, 
asegurando el almacenamiento persistente mediante volúmenes de Docker.

\item \textbf{Motor de búsqueda (\textit{Meilisearch})}: Nodo independiente para la indexación y búsqueda 
