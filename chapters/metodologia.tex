...
\begin{itemize}
    ...
    \item \textbf{Límites WIP (Work In Progress):} Control del número de tareas simultáneas para mantener foco y evitar dispersión.
...
El diagrama de Gantt generado (ver Figura \ref{fig:gantt}) muestra la distribución temporal de las diferentes fases del proyecto: análisis y diseño inicial, desarrollo de módulos principales, integración, pruebas y documentación final.
...
\hline
\multicolumn{4}{|p{0.9\textwidth}|}{\textbf{Plan de mitigación:} En lugar de hacer cruces manuales o exactos, se programarán mecanismos de Fuzzy Matching (búsqueda aproximada de cadenas) utilizando librerías específicas como FuzzyWuzzy o TheFuzz, complementados con diccionarios manuales únicamente para los casos extremos.} \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Estándares de estilo:} Dado que el stack principal se basa en Python (tanto para ML como para Django), se utilizará un linter (Flake8 o Black) para forzar el cumplimiento estricto de la guía de estilo PEP 8.
\end{itemize}

\subsection{4.7 Análisis de tiempos, costes y desviaciones}

Tal y como se estableció en la planificación temporal, la jornada de trabajo estándar para este proyecto se fijó en 4 horas diarias, adaptándose a la realidad de la carga lectiva del grado. Durante el desarrollo, esta premisa se ha cumplido de forma general, con la excepción del periodo vacacional de Navidad (del 24 de diciembre al 30 de enero). La mayor disponibilidad de tiempo en esas fechas permitió acelerar el ritmo de trabajo, logrando completar tareas en una menor cantidad de días calendario, aunque el volumen de horas de trabajo invertidas se mantuvo acorde a las estimaciones.
...
Como se observa en los datos, el esfuerzo total del proyecto solo sufrió un incremento de 20 horas netas (equivalentes a 5 jornadas de trabajo de 4 horas). A nivel de calendario, la finalización se desplazó hasta el 7 de abril. Gracias al gran margen de holgura planificado desde el principio, este desplazamiento temporal fue absorbido íntegramente sin poner en riesgo la entrega ni la defensa del Trabajo de Fin de Grado.

\subsubsection{Comparativa de esfuerzo por paquetes de trabajo (EDT)}
...
\toprule
\textbf{ID} & \textbf{Paquete de Trabajo (EDT)} & \textbf{Horas Previstas} & \textbf{Horas Reales} & \textbf{Desviación} \\
\midrule
...
Para calcular el coste del desarrollo, se ha tomado como referencia el salario medio de un perfil profesional de Analista programador J2EE / Web, considerado representativo del perfil estándar de desarrollo software.

Los datos salariales provienen de un informe de precios y perfiles informáticos disponible en la plataforma de ETSII Universidad de Sevilla. En dicho documento, elaborado a partir de una consulta preliminar de mercado de la Junta de Andalucía, se establece para este perfil un rango de precios/hora que sitúa la media en torno a los 30–33 €/hora, siendo el valor medio aproximado utilizado en este proyecto de 31 €/hora.

A diferencia de estimaciones basadas en salarios brutos anuales, este valor ya incorpora una aproximación realista del coste de mercado (incluyendo costes indirectos, variabilidad entre empresas y experiencia profesional), por lo que no es necesario aplicar factores correctores adicionales.
...
\begin{itemize}
    \item Amortización del equipo informático utilizado. Se considera un ordenador con un coste aproximado de 1.200 € y una vida útil de 4 años. Imputando un uso parcial al proyecto, se estima un coste fijo de 100 €.
\end{itemize}
...
\begin{itemize}
    ...
    \item No obstante, gracias a la disponibilidad de licencias educativas y el programa Azure for Students, el coste real aplicado al proyecto ha sido de 0 €, lo que supone un ahorro aproximado de 162 € durante los tres meses de desarrollo.
\end{itemize}
...
CAPEX (Amortización de equipo) & 100,00 € & 100,00 € \\
OPEX (Licencias e Infraestructura Cloud) & 0,00 €* & 0,00 €* \\
\midrule
COSTE TOTAL DEL PROYECTO & 11.384,00 € & 12.004,00 € \\
\bottomrule
\end{tabularx}
\end{center}

\noindent (Nota: Sin la aplicación de licencias educativas, el OPEX ascendería a 162 € en total).

El coste final de desarrollo del Trabajo de Fin de Grado asciende a 12.004,00 €, lo que supone un incremento de 620,00 € respecto a lo estimado inicialmente, derivado principalmente del aumento de horas en las fases de entrenamiento de modelos y desarrollo.
...
\textbf{Reserva de contingencia:} 1.200,40 €
