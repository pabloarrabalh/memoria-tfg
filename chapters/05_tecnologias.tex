...
REST. Esto encaja con un frontend desacoplado que consume endpoints, evitando dependencia 
...
En cuanto a seguridad, Django incorpora protecciones y buenas prácticas documentadas para mitigar riesgos 
...
\begin{itemize}
    \item \textbf{Soporte para Django Channels:} Redis funciona como la \textit{channel layer} necesaria para gestionar la comunicación asíncrona de los WebSockets, permitiendo el envío de notificaciones en tiempo real (eventos de partidos o interacciones sociales).
...
jugador en partido) y columnas representan variables. Esto se conoce como enfoque tabular es coherente con la 
...
statsbomb, soccerdata), finalmente descartadas por limitaciones prácticas (cobertura, 
...
\subsection{5.8 APIs y otras herramientas de soporte}
...
\item \textbf{Capa de mensajería y caché (Redis):} Nodo independiente basado en \texttt{redis:7-alpine} para el soporte de tiempo real y optimización de lecturas de la API.
...
\item \textbf{Motor de Búsqueda (Meilisearch):} Nodo independiente para la indexación y búsqueda 
