...
\section*{9. Implementación y desarrollo}
 
\subsection{9.1 Implementación del backend con Django}
 
Durante la implementación del backend con \textit{Django}, uno de los principales retos fue
...
\subsubsection{9.1.1 Serializers}
 
Un serializer en DRF es una clase que define cómo se transforma un objeto de \textit{Python}
...
\begin{lstlisting}[language=Python,caption={Validación de registro con verificación de contraseñas en Django REST Framework}]
...
\begin{lstlisting}[language=Python,caption={Serializer anidado con campos dinámicos usando SerializerMethodField}]
...
\begin{lstlisting}[language=Python,caption={Resolución ambigua de relaciones sociales en serializers mediante context}]
def _amigo(self, obj):
...
\subsubsection{9.1.2 Implementación de la lógica de negocio}
...
\subsubsection{9.1.2.1 Gestión de identidad y entorno social}
...
\subsubsection{9.1.2.2 Motor de búsqueda, clasificación y agregación de estadísticas}
...
\begin{lstlisting}[language=Python,caption={Cálculo dinámico de percentiles relativos por posición}]
...
al contenedor local o visitante, permitiendo que el frontend renderice una lista cronológica de lo ocurrido en cada encuentro de la jornada seleccionada.
 
\begin{lstlisting}[language=Python,caption={Reconstrucción de eventos del partido mediante cruce relacional}]
...
\subsubsection{9.1.2.3 Detalles de los equipos}
...
un objeto \texttt{rival\_info} dinámico que permite comparar racha y H2H (Head-to-Head) en tiempo real.
 
\begin{lstlisting}[language=Python,caption={Construcción de objeto de análisis comparativo entre equipos rivales}]
rival_info = {
    'nombre': rival.nombre,
...
\subsubsection{9.1.2.4 Perfil del jugador}
...
\subsubsection{9.1.2.5 Generación y explicación de predicciones}
...
\subsubsection{9.1.2.6 Gestión de alineaciones}
 
\texttt{plantilla.py} actúa como el motor de construcción de alineaciones,
...
\begin{lstlisting}[language=Python,caption={Detección de eventos en vivo con sistema de claves únicas para evitar redundancias}]
for stat in stats:
...
Finalmente, como medida de aviso, la API rastrea la participación
...
\begin{lstlisting}[language=Python,caption={Sistema de alertas para jugadores con bajo minutaje}]
pocos_minutos_set = {
...
\subsubsection{9.1.2.7 Orquestación}
 
Dado que el cálculo de percentiles y rankings estadísticos requiere procesar miles de registros de \texttt{EstadisticasPartidoJugador},
...
\subsubsection{9.1.3 Arquitectura reactiva y optimización de rendimiento}
...
\subsubsection{9.1.3.1 Automatización mediante Signals}
 
Para mantener el desacoplamiento del código, se utilizó el patrón Observer mediante los \texttt{Django Signals}. Esto permite que el sistema reaccione a
...
\subsubsection{9.1.3.2 Comunicación en tiempo real con WebSockets}
...
\begin{lstlisting}[language=Python,caption={Signal de Django para emitir notificaciones en tiempo real vía WebSockets}]
...
\subsubsection{9.1.3.3 Estrategia de caching y optimización de agregaciones}
...
\subsubsection{9.1.4 Arranque y ciclo de vida}
...
\subsubsection{9.1.4.1 Inicialización asíncrona y gestión de cold start}
...
\begin{lstlisting}[language=Python,caption={Inicialización asíncrona en hilo durante startup de Django}]
def ready(self):
...
\subsubsection{9.1.4.2 Pipeline de arranque}
...
\item Carga masiva de datos: si la comprobación falla, invoca el módulo de scraping para poblar la base de datos desde cero. Un detalle técnico relevante es la desconexión temporal
de Signals durante esta fase; esto evita que se disparen miles de hilos de predicción innecesarios durante una carga masiva, optimizando el uso de CPU y memoria.
...
\subsubsection{9.1.4.3 Pre-carga de módulos de Machine Learning}
...
\subsection{9.2 Implementación del scraping de datos}
...
\subsubsection{9.2.1 Origen de los datos y fuentes}
...
\subsubsection{9.2.2 Gestión de barreras técnicas y bloqueos}
...
\subsubsection{9.2.3 Algoritmo de reintentos}
 
Para garantizar la robustez de las descargas en \textit{FutbolFantasy}, se desarrolló
...
\begin{lstlisting}[language=Python,caption={Algoritmo de reintentos para descargas robustas}]
...
\subsubsection{9.2.4 Módulos técnicos y responsabilidades}
...
\begin{itemize}
\item Parseo de elementos ocultos: los datos de rankings en FBRef se cargan de forma asíncrona o se encuentran dentro de comentarios HTML. \texttt{roles.py} implementa una técnica de extracción de comentarios mediante BeautifulSoup para acceder a esta información.
...
\begin{lstlisting}[language=Python,caption={Extracción de nodos comentario HTML con BeautifulSoup para datos ocultos}]
...
\subsubsection{9.2.5 Identificación de roles}
...
\subsubsection{9.2.6 Orquestación del scraping}
...
\subsection{9.3 Implementación del matching de entidades}
...
\subsubsection{9.3.1 Alias por temporada y casos especiales}
...
\subsubsection{9.3.2 Generación de propuestas de matching}
...
\item Se genera una lista de propuestas que incluye el score de confianza y metadatos como posición y minutos.
\end{itemize}
...
\subsubsection{9.3.3 Lógica principal y Fuzzy Matching}
...
\subsubsection{9.3.3.1 Justificación técnica: ¿Por qué WRatio frente a Levenshtein?}
...
\subsubsection{9.3.4 Resolución de conflictos y desempates}
...
\subsubsection{9.3.5 Integración con el flujo de scraping}
...
\subsection{9.4 Implementación del sistema de Machine Learning}
...
\subsubsection{9.4.1 Arquitectura del BaseTrainer}
...
\begin{lstlisting}[language=Python,caption={Clase base abstracta para trainers de modelos ML con split temporal}]
...
\subsubsection{9.4.2 Estrategia de validación y Grid Search}
...
\subsubsection{9.4.3 Robustez y persistencia}
...
siempre los cuatro modelos entrenados, incluso en escenarios donde modelos lineales como Ridge presentaran
un MAE ligeramente inferior.
 
\subsubsection{9.4.4 Criterios de evaluación y selección de métricas}
...
\subsection{9.5 Implementación del feature engineering}
...
\subsubsection{9.5.1 Generación automatizada de variables temporales}
...
\subsubsection{9.5.2 Heurísticas de limpieza}
...
\subsubsection{9.5.3 Especialización asimétrica por posición}
...
y combinaciones de forma y disponibilidad. En defensas, se enfatizan entradas, intercepciones, despejes y duelos. En mediocampistas, el foco se desplaza
hacia pases clave, asistencias, regates y continuidad de minutos. En delanteros, se da más peso a goles, tiros a puerta, penaltis y eficiencia ofensiva.
...
\subsubsection{9.5.4 Inyección de contexto externo}
...
Este bloque fue especialmente útil para introducir señales sin depender solo de estadísticas acumuladas. Por ejemplo,
...
\subsubsection{9.5.5 Selección de características mediante Spearman}
...
\subsection{9.6 Implementación de IA Explicable (XAI)}
...
\subsubsection{9.6.1 Fundamentos de la metodología SHAP}
...
\subsubsection{9.6.2 Motor de explicación unificado}
...
instancia un \texttt{TreeExplainer}; si es un modelo lineal (Ridge o ElasticNet), utiliza un \texttt{LinearExplainer}.
...
\subsubsection{9.6.3 Integración en el flujo de la API}
...
\subsubsection{9.6.4 Seguridad deportiva y estrategia híbrida}
...
\subsection{9.7 Implementación del frontend}
...
\subsubsection{9.7.1 Estructura general de la interfaz}
...
\subsubsection{9.7.2 Gestión del estado global}
...
\subsubsection{9.7.3 Problemas reales de sincronización y carga}
...
\subsubsection{9.7.4 Elementos destacados de la interfaz}
...
\subsection{9.8 Principales problemas enfrentados}
...
\subsubsection{9.8.1 Optimización del despliegue}
...
Dada la magnitud del dataset y la complejidad del pipeline de scraping (que, como se ha detallado en el punto 9.2, incluye miles de registros estadísticos), este proceso de
...
\textbf{Solución:} Se implementó una lógica de integridad en el componente \texttt{apps.py} (explicada en el punto 9.1.4). El sistema ahora realiza una auditoría de persistencia
...
\subsubsection{9.8.2 Ambigüedad en el matching de entidades y barreras de scraping}
...
\textbf{Solución:} Se abandonó la comparación simple por cadenas de texto en favor de la lógica difusa y el matching por contexto de partido (detallado en el punto 9.3). Además, para evadir los bloqueos, se diseñó
la estrategia de backoff exponencial y la gestión de descargas locales controladas para los datos históricos más pesados.
 
\subsubsection{9.8.3 Refactorización arquitectónica: de Django Templates a React}
...
la correcta gestión del estado global (como se analiza en el punto 9.7) y una experiencia de usuario fluida que la automatización no pudo alcanzar.
 
\subsubsection{9.8.4 Integridad predictiva: detección y corrección de Data Leakage}
...
Esto provocó un fenómeno de data leakage: el modelo ``predecía'' el pasado utilizando información del futuro, arrojando métricas de precisión extraordinariamente altas que no se correspondían con la realidad. Al intentar
predecir jornadas nuevas sin datos almacenados, el rendimiento caía drásticamente.
 
\textbf{Solución:} Se rediseñó por completo el motor de entrenamiento en el \texttt{BaseTrainer} (ver punto 9.4.2), imponiendo un corte temporal estricto. Se aseguró que para predecir la jornada $n$, el modelo solo tuviera acceso a datos
...
\subsubsection{9.8.5 Elección del motor de búsqueda}
...
\subsubsection{9.8.6 Despliegue en producción}
